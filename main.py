...
async def submit(request: Request):
    ...
    collection_region = form.get("collection_region") or ""  # not stored in DB schema yet
    collection_person = form.get("collection_contact_name") or ""
    collection_number = form.get("collection_contact_number") or ""
    collection_email = form.get("collection_email") or ""
    ...
    delivery_email = form.get("delivery_email") or ""
    ...
    generate_pdf({
        ...
        "collection_email": collection_email,  # ✅ add this
        "delivery_company": delivery_company,
        "delivery_address": delivery_address,
        "delivery_person": delivery_person,
        "delivery_number": delivery_number,
        "delivery_email": delivery_email,  # ✅ add this
    ...
    # Email: choose collection_email first, else delivery_email
    to_email = collection_email or delivery_email
    if to_email:
        subject = f"Hazmat Collection Confirmation • {reference_number}"
        body = (
            "Dear Valued Client,\n\n"
            "We are pleased to confirm that your collection request has been successfully received by the Hazmat Operations team. "
            "Our specialists are already reviewing the details, and your shipment will be attended to shortly.\n\n"
            "Warm regards,\nHazmat Global Logistics Team\n"
        )
        attachments = [pdf_path] + uploaded_paths
        try:
            send_confirmation_email(to_email, subject, body, attachments, cc_email=OPS_CC)
        except Exception as e:
